...
def get_full_path(db: Session, farm: Farm):
    ...
    while curr_region:
        ...
        if curr_region.parent_id:
            curr_region = (
                db.query(Region).filter(Region.id == curr_region.parent_id).first()
            )
        else:
            break
    return path
...
def main():
    ...
    try:
        ...
        for f in files:
            print(
                f"Deleting folder '{f['name']}' (ID: {f['id']}, Trashed: {f.get('trashed')}) from Drive..."
            )
            try:
    ...
    for farm in farms:
        ...
        try:
            res_store = (
                service.files()
                .list(q=query_store, spaces="drive", fields="files(id, name)")
                .execute()
            )
            ...
            query_origin = (
                f"name='origin' and '{store_id_drive}' in parents and trashed=false"
            )
            res_origin = (
                service.files()
                .list(q=query_origin, spaces="drive", fields="files(id)")
                .execute()
            )
            ...
            res_files = (
                service.files()
                .list(q=query_files, spaces="drive", fields="files(id, name)")
                .execute()
            )
