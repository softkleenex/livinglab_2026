...
start = time.time()
fetch_json(
    "http://localhost:8000/api/user/context",
    json.dumps(
...
req_guest = urllib.request.Request(
    "http://localhost:8000/api/ingest", data=body_guest.encode("utf-8")
)
...
print(
    f"  -> Trust Index: {guest_data['entry']['trust_index']}% (Guest Penalty Applied)"
)
...
req_auth = urllib.request.Request(
    "http://localhost:8000/api/ingest", data=body_auth.encode("utf-8")
)
...
hierarchy_data = fetch_json(
    f"http://localhost:8000/api/hierarchy/explore?path={urllib.parse.quote(street_path)}"
)
...
delete_req = urllib.request.Request(
    f"http://localhost:8000/api/ingest/delete?path={urllib.parse.quote(path)}&hash_val={guest_hash}",
    method="DELETE",
)
urllib.request.urlopen(delete_req, context=context)
print(f"  -> Deletion Success! ({time.time() - start:.2f}s)")

hierarchy_data_after = fetch_json(
    f"http://localhost:8000/api/hierarchy/explore?path={urllib.parse.quote(street_path)}"
)
