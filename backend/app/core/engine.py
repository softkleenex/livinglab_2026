...
class HierarchyEngine:
    def get_object(self, db, path_list):
        if not path_list or path_list[0] == "전체 (Root)":
            cities = db.query(Region).filter(Region.parent_id is None).all()
        ...
        for i, p in enumerate(path_list):
            if i == len(path_list) - 1:
                farm = (
                    db.query(Farm)
                    .filter(Farm.name == p, Farm.region_id == parent_id)
                    .first()
                )
                if farm:
                    is_store = True
        ...
        if is_store:
            ...
            entries = (
                db.query(DataEntry)
                .options(joinedload(DataEntry.farm))
                .filter(DataEntry.farm_id == curr_obj.id)
            ...
            children_regions = (
                db.query(Region).filter(Region.parent_id == curr_obj.id).all()
            )
            ...
            if not current_path_str or current_path_str == "전체 (Root)":
                ...
                entries = (
                    db.query(DataEntry)
                    .options(joinedload(DataEntry.farm))
                    .filter(DataEntry.location_path.like(f"{current_path_str}%"))
                    .order_by(DataEntry.created_at.desc())
                    .limit(100)
                    .all()
                )
    ...
    def add_value_bottom_up(self, db, path_list, value_added):
        """
        [BUSINESS_RULES.md - 1.3 성능 고려사항]
        롤업 연산은 원래 비동기 스케줄링(Background Worker)을 통해 처리되어야 합니다.
        본 메서드는 현재 아키텍처 단순성을 위해 동기적으로 구현되었으나,
        운영 환경에서는 비동기 큐(예: FastAPI BackgroundTasks 등)를 통해 
        Non-blocking으로 실행됨을 시뮬레이션 합니다.
        """
        parent_id = None
        regions = []
        if path_list[0] == "전체 (Root)":
        ...
        for rid in region_ids:
            locked_r = (
                db.query(Region).filter(Region.id == rid).with_for_update().first()
            )
            if locked_r:
