...
class Region(Base):
    ...
    __table_args__ = (
        UniqueConstraint("name", "parent_id", name="uix_region_name_parent"),
    )
    id = Column(Integer, primary_key=True, index=True)
    ...
    farms = relationship("Farm", back_populates="region", cascade="all, delete-orphan")
    products = relationship(
        "Product", back_populates="region", cascade="all, delete-orphan"
    )
...
class Farm(Base):
    __tablename__ = "farms"
    __table_args__ = (
        UniqueConstraint("name", "region_id", name="uix_farm_name_region"),
    )
    id = Column(Integer, primary_key=True, index=True)
    ...
    entries = relationship(
        "DataEntry", back_populates="farm", cascade="all, delete-orphan"
    )
...
class Product(Base):
    ...
    category = Column(
        String, index=True
    )  # 'synthetic_data', 'machinery', 'raw_data', 'co-purchase'
    title = Column(String)
    ...
    matchings = relationship(
        "Matching", back_populates="product", cascade="all, delete-orphan"
    )
...
class SyntheticData(Base):
    ...
    data_type = Column(
        String, index=True
    )  # 'yield_prediction', 'oversupply_risk', 'heat_stress'
    raw_sources = Column(
        JSON, default=list
    )  # e.g., [{"source": "기상청", "type": "단기예보"}]
...
class DataAPIKey(Base):
    """데이터 마켓플레이스에서 구매한 데이터를 연동하기 위한 API 키"""
