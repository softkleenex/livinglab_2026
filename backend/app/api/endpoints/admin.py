...
async def demo_inject(
    ...
    try:
        ...
        mock_insights = [
            {
                "date": "2026-04-05",
                "text": "합성 데이터 예측: 다음 주 강수량 부족으로 인해 토양 수분 지수가 경고 수준에 도달할 예정입니다. 스프링클러 가동 시간을 20% 늘리는 것을 권장합니다.",
                "trust": 88.5,
            },
            {
                "date": "2026-04-07",
                "text": "합성 데이터 분석: 폭염 대비 관수 시스템 용량이 부족할 것으로 예상됩니다. 내일까지 예비 용수 확보가 필요하므로 주변 농가와 자원 공유 일정을 협의하세요.",
                "trust": 92.0,
            },
            {
                "date": "2026-04-08",
                "text": "AI 멀티모달 분석 (비전): 업로드하신 작물 이미지를 스캔했습니다. 잎사귀의 갈변 현상이 관찰되며, 질소 비료 결핍 또는 초기 병충해일 가능성이 있습니다. 추가 토양 검사를 권장합니다.",
                "trust": 95.5,
                "link": "https://drive.google.com/file/d/1Xdvq-HOVBOdaS0oalgrVXrXSvi0AYonQ/view?usp=drivesdk",
            },
        ]
        ...
        for i, m in enumerate(mock_insights):
            ...
            entry_hash = hashlib.sha256(
                (m["text"] + str(random.random())).encode()
            ).hexdigest()
            ...
            manager.broadcast(
                {
                    "type": "update",
                    "path": path_list,
                    "value_added": total_effective_value,
