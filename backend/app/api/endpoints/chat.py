...
from app.api.endpoints.ingest import (
    sync_drive_delete,
    sync_drive_modify,
    sync_drive_upload,
)
...
def sync_chat_log_drive_upload(path_list, payload_industry, message, reply):
    try:
        ...
        if drive_service:
            ...
            txt_metadata = {
                "name": f"Copilot_Log_{now_str}.txt",
...
async def chat_with_copilot(
    ...
    parent_avg = parent_obj["metadata"].get("total_value", 0) // max(
        1, parent_obj["metadata"].get("nodes", 1)
    )
    ...
    try:
        ...
        if action_type == "NONE":
            ...
            chat_res = await asyncio.to_thread(
                client.models.generate_content, model=model_name, contents=chat_prompt
            )
            reply = chat_res.text
            ...
            if user["role"] == "guest":
                ...
                actual_entry = next(
                    (e for e in entries if e["hash"].startswith(target_hash)), None
                )

                if actual_entry:
                    entry_to_del = (
                        db.query(DataEntry)
                        .filter(DataEntry.hash_val == actual_entry["hash"])
                        .first()
                    )
                    if entry_to_del and (
                        entry_to_del.farm.owner_id == user["user_id"]
                        or user["role"] == "admin"
                    ):
                        penalty = -entry_to_del.effective_value
                        del_path_list = [
                            p for p in entry_to_del.location_path.split("/") if p
                        ]
                        ...
                        background_tasks.add_task(
                            sync_drive_delete, short_hash_to_del, drive_link_to_del
                        )
                        ...
                        reply = (
                            "⚠️ [시스템] 삭제 권한이 없거나 데이터를 찾을 수 없습니다."
                        )
                else:
            ...
            if user["role"] == "guest":
                ...
                if actual_entry:
                    ...
                    if entry_to_mod and (
                        entry_to_mod.farm.owner_id == user["user_id"]
                        or user["role"] == "admin"
                    ):
                        ...
                        # Fix Data Drift
                        background_tasks.add_task(
                            sync_drive_modify, entry_to_mod.hash_val[:8], new_text
                        )

                        reply = "✨ [시스템] 데이터가 성공적으로 수정되었습니다."
                    else:
                        reply = (
                            "⚠️ [시스템] 수정 권한이 없거나 데이터를 찾을 수 없습니다."
                        )
                else:
                    reply = "⚠️ [시스템] 권한 밖이거나 찾을 수 없는 해시값입니다."
            ...
            if user["role"] == "guest":
                ...
                reply = (
                    "✨ [시스템] 새로운 데이터가 성공적으로 추가 및 자산화되었습니다."
                )

        # Delegate Drive API to Background Task
        background_tasks.add_task(
            sync_chat_log_drive_upload,
            path_list,
            payload.industry,
            payload.message,
            reply,
