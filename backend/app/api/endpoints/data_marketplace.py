...
async def export_synthetic_yield_data(
    ...
    """
    try:
        query = db.query(SyntheticData).filter(
            SyntheticData.data_type == "yield_prediction"
        )
        if region:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": "attachment; filename=mdga_synthetic_yield_export.csv"
            },
        )
...
async def export_ai_ready_vision_data(
    ...
    try:
        query = db.query(DataEntry).filter(
            DataEntry.drive_link.isnot(None)
        )  # Assuming items with drive_links have images/vision context
        if industry:
            query = query.filter(DataEntry.industry == industry)

        results = query.order_by(DataEntry.created_at.desc()).limit(limit).all()
        ...
        for r in results:
            ...
            # Anonymize location_path: strip the Leaf (Farm) part
            path_parts = r.location_path.split("/")
            if len(path_parts) > 1:
                anonymized_path = "/".join(path_parts[:-1])
            else:
                anonymized_path = r.location_path

            row = {
                "id": r.id,
                "location_path": anonymized_path,
                "industry": r.industry,
                "insights": r.insights,
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": "attachment; filename=mdga_ai_ready_vision_logs.jsonl"
            },
        )
    except Exception as e:
        traceback.print_exc()
