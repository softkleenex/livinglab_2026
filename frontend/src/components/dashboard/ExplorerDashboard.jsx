...
export default function ExplorerDashboard({ explorerData, currentPath, goBack, setCurrentPath, navigateTo }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-5xl mx-auto w-full pb-8 px-2 sm:px-0">
      {/* Header with Nav */}
          ...
          {explorerData.children && explorerData.children.length > 0 ? (
            <div className="grid grid-cols-1 gap-3">
              {explorerData.children.sort((a,b) => b.pulse - a.pulse).map((child, idx) => (
                <div key={child.name} onClick={() => navigateTo(child.name)} className="bg-[#101725] p-4 rounded-2xl border border-slate-800/80 hover:border-blue-500/50 hover:bg-[#121A2A] cursor-pointer group transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between relative overflow-hidden gap-4">
                  {idx === 0 && (
                  ...
                  <div className="flex items-center justify-between w-full sm:w-auto sm:justify-end gap-6 sm:gap-4 shrink-0 border-t sm:border-t-0 border-slate-800/60 pt-3 sm:pt-0">
