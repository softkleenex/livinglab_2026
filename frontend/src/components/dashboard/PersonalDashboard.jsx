...
export default function PersonalDashboard({ personalData, userContext, handleExportCSV, setShowReport, handleDeleteEntry, handleDemoInject }) {
  ...
  return (
    <motion.div key="personal" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-4xl mx-auto w-full pb-8 px-2 sm:px-0">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div className="space-y-2">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-white leading-tight break-keep">{personalData.store.name}</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          {isB2B ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-[#101725] p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-blue-500/30 transition-colors">
                ...
                <motion.p key={personalData.store.total_value} initial={{ scale: 1.1, color: '#34d399' }} animate={{ scale: 1, color: '#34d399' }} className="text-2xl sm:text-3xl font-bold text-emerald-400">
              ...
              <div className="bg-[#101725] p-5 rounded-2xl border border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.1)] relative group">
                ...
                <motion.p key={personalData.store.pulse} initial={{ scale: 1.5, color: '#10b981' }} animate={{ scale: 1, color: '#10b981' }} className="text-2xl sm:text-3xl font-bold text-white mb-2">
          ...
          <div className="bg-[#101725] p-6 rounded-2xl border border-slate-800 shadow-lg flex flex-col sm:flex-row gap-6 items-center">
          ...
          {personalData.store.entries.length === 0 ? (
            ...
            <div className="space-y-3">
              {[...personalData.store.entries].reverse().map((entry, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.1 }} className="bg-[#101725] p-4 sm:p-5 rounded-2xl border border-slate-800 shadow-md relative group hover:border-slate-600 transition-colors">
