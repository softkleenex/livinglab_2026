...
## 5. 🛡️ 데이터 거버넌스 및 신뢰도 검증 엔진 (Trust & Governance)

**"모든 데이터에는 무게(가중치)가 있고, 삭제에는 책임이 따른다."**

- **개별 신뢰 지수 (Trust Index)**: 
  - 피딩된 모든 데이터 엔트리는 이미지 첨부 여부, 내용의 구체성에 따라 **독립적인 신뢰도 점수(ex: 95.5%)**를 부여받습니다.
  - 이 신뢰도는 단순 표기용이 아니며, 매장이 획득하는 **최종 자산 가치(Effective Value)를 계산하는 핵심 가중치**로 사용됩니다.
- **데이터 롤백 및 패널티 (Roll-back System)**: 
  - 사용자는 자신이 올린 데이터를 언제든 휴지통(🗑️) 버튼을 통해 삭제할 수 있습니다.
  - 데이터가 삭제될 경우, 백엔드는 해당 엔트리를 해시(`hash`) 값으로 추적하여 DB에서 제거하고, 그 데이터가 기여했던 가치(Value)와 맥박(Pulse)만큼 **하향 전파(Bottom-up Roll-down)**를 실행합니다.
  - 동시에 매장의 평균 신뢰도를 하락시켜 데이터 어뷰징을 시스템적으로 억제합니다.
- **데이터 스코핑 (Data Scope)**: 
  - 저장되는 모든 데이터는 계층 구조의 뎁스(Depth)에 따라 **`MY STORE (매장 전용 데이터)`**와 **`PUBLIC DATA (상권 공용 데이터)`**로 엄격히 분리되어 저장 및 시각화(라벨링)됩니다.

---

## 6. 🧭 페르소나별 뷰와 어플리케이션 플로우 (App Flow)
