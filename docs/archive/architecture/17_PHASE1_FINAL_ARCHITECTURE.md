...
## 8. SRE (사이트 신뢰성 엔지니어링) 및 극한의 마이크로 최적화

초기 기능 개발이 완료된 이후, 최대 5,000명의 가상 시니어 엔지니어 페르소나(Swarm)를 투입하여 수백 차례에 걸친 현미경 단위의 강박적인 코드 리뷰와 리팩토링을 수행했습니다. 

- **데드락(Deadlock) 및 N+1 쿼리 박멸:** 데이터를 상위 계층으로 롤업할 때 발생하는 동시성 충돌을 막기 위해 모든 락(Lock)을 ID 기준 오름차순(Top-Down) 정렬 방식으로 통일했습니다. 또한 SQLAlchemy의 `joinedload`를 전면 도입하여 하위 상점 객체 호출 시 발생하는 N+1 쿼리를 단일 조인 쿼리로 병합했습니다.
- **클라우드 인프라(DevOps) CI/CD 구축:** 환경과 무관하게 백엔드를 무한 확장(Scale-out)할 수 있도록 `Dockerfile` 및 `docker-compose.yml`을 작성하였으며, Github Actions를 연동하여 `.github/workflows/pytest.yml`이 코드를 Push할 때마다 롤업 엔진의 단위 테스트 엣지 케이스를 자동 검증합니다.
- **기업용 방어 로직 (Rate Limiting & BOLA 방어):** `slowapi`를 미들웨어로 장착하여 악의적인 챗봇 API DDoS 공격을 차단하고, 모든 엔드포인트에 `user.get("role") != "admin"` 등 철저한 IDOR/BOLA 권한 인증 로직을 적용하여 타인의 상점 데이터를 변조하거나 DB를 포맷하는 백도어성 취약점을 100% 폐쇄했습니다.
- **프론트엔드 React 렌더링 최적화:** 지도(`DigitalTwinMap.jsx`)와 같은 헤비한 UI 컴포넌트가 불필요하게 다시 렌더링되지 않도록 `React.memo`와 `useMemo`를 극한으로 적용하였으며, 모든 팝업 요소에 `aria-modal="true"`, `role="dialog"`와 이미지 `alt` 속성을 추가하여 모자람 없는 웹 접근성(A11y) 만점을 달성했습니다.
- **전역 에러 핸들링 및 의존성 주입 최적화:** FastAPI의 미들웨어에 B2B 표준 전역 에러 핸들러(`exception_handler`)와 정규화된 `logging` 객체를 탑재하여 내부 스택 트레이스 노출을 차단(Obfuscation)했습니다. 더불어 인증 토큰 미들웨어가 낭비하던 DB `SessionLocal` 누수를 막아 커넥션 풀의 자원 고갈을 방지했습니다.
