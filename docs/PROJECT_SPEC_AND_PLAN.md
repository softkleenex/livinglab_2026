...
### 2.1 핵심 비즈니스 시나리오 (User Flow)
*   **데이터 입력:** 사용자가 영농일지 텍스트와 사진을 업로드. 사진은 **Google Drive**에 즉시 업로드되고 해당 링크가 DB에 저장됨.
*   **멀티모달 파싱:** Gemini API가 데이터를 추출하여 JSON 포맷으로 반환.
*   **공간 매핑 및 적재:** 계층 구조 매핑 후 **Supabase(PostgreSQL)**에 적재.
*   **시뮬레이션 및 합성:** 미래 기후 조건 하의 작황 변동성을 시뮬레이션.
*   **시각화 및 판매:** Twin Map에 시각화하고 마켓플레이스에서 판매.
